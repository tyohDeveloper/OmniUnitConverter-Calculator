...
---
The rule: `scripts/verify-build.mjs` enforces gzip size ≤ baseline × 1.05 (baseline re-recorded 2026-07-20 at 493.8 kB gzip; ceiling ~518 kB; current build ~448 kB gzip, so ~70 kB headroom as of late July 2026). Parallel data additions can each pass locally but stack past the ceiling on merge, so the clean tree may already be failing before your change — always build the untouched tree first to separate pre-existing growth from your own delta, audit that the growth is legitimate data, then re-baseline with a note.
