...
|---|---|
| **VIEW** | `client/src/features/**/*.tsx`, `client/src/components/**/*.tsx` |
| **CONTROLLER** | `client/src/components/unit-converter/hooks/**`, `client/src/features/unit-converter/app/**` |
| **STATE** | `client/src/components/unit-converter/state/**`, `client/src/components/unit-converter/context/**` |
| **PURE** | `client/src/lib/**` |
| **PURE-CORE** | `client/src/lib/calculator/**`, `client/src/lib/units/**` |
| **DATA** | `client/src/data/**` |

**Target:** standalone single-file HTML5.

**Reference implementation.** This repository is where these standards were
established. `docs/perplexity/architecture-standards.md` (v1.7) remains as the
OmniUnit-specific elaboration — the `data-testid` grammar, the polyglot XHTML
output contract, the minimization pass, and the locale-data rules are stated
there in path-literal detail. Its §3 limits are identical to §3 here.
