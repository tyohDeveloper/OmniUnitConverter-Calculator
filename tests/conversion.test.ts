...
import { CONVERSION_DATA, convert } from "@/lib/conversion-data";
...
describe("Unit Ordering - SI Base First, Then Ascending Factor", () => {
  ...
  describe("Specific Category Order Verification", () => {
    it("Mass should be ordered: kg, eV/c², Da, mcg, g, oz, oz_t, lb_t, lb, st, slug, ton_us, t, ton_uk", () => {
      const mass = CONVERSION_DATA.find(c => c.id === "mass");
      const ids = mass?.units.map(u => u.id);
      
      expect(ids?.[0]).toBe("kg");
      expect(ids?.[1]).toBe("ev_c2");
      expect(ids?.[2]).toBe("dalton");
      expect(ids?.[3]).toBe("mcg");
      expect(ids?.[4]).toBe("g");
      expect(ids?.[5]).toBe("oz");
      expect(ids?.[6]).toBe("oz_t");
      expect(ids?.[7]).toBe("lb_t");
      expect(ids?.[8]).toBe("lb");
    });

    it("Length should be ordered: m, Å, thou, in, ft, ft:in, yd, mi, nmi, AU, ly, pc", () => {
      const length = CONVERSION_DATA.find(c => c.id === "length");
      const ids = length?.units.map(u => u.id);
      
      expect(ids?.[0]).toBe("m");
      expect(ids?.[1]).toBe("angstrom");
      expect(ids?.[2]).toBe("thou");
      expect(ids?.[3]).toBe("in");
    });
